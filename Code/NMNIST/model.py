...
class SpikingPatchStemNMNIST(nn.Module):
    """32x32 N-MNIST frames -> 8x8 spiking feature map (64 tokens)."""
    ...
    def __init__(
        ...
        # Preserve the original N-MNIST spatial schedule:
        # 32 -> 32 -> 32 -> 16 -> 8.
...
class ALSMNMNIST(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.patch_embed = SpikingPatchStemNMNIST(
            img_size=img_size,
...
def alsm_nmnist(**kwargs) -> ALSMNMNIST:
    return ALSMNMNIST(**kwargs)


def spikformerLSM(**kwargs) -> ALSMNMNIST:
    """Backward-compatible factory for the original N-MNIST training API."""
    kwargs = dict(kwargs)
    img_h = kwargs.pop("img_size_h", kwargs.pop("img_size", 32))
    img_w = kwargs.pop("img_size_w", img_h)
    if img_h != img_w:
        raise ValueError("The improved N-MNIST model expects square inputs.")
    ...
    return ALSMNMNIST(img_size=img_h, **kwargs)


__all__ = ["ALSMNMNIST", "alsm_nmnist", "spikformerLSM"]
