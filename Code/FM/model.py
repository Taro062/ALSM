...
class ConvBNPoolLIF(nn.Module):
    """CML-style downsampling unit used inside the spiking patch stem."""
...
class SpikingPatchStemFashionMNIST(nn.Module):
    """28x28 Fashion-MNIST images -> 7x7 spiking feature map (49 tokens)."""
    ...
    def __init__(
        ...
        # Preserve the original Fashion-MNIST spatial schedule:
        # 28 -> 28 -> 28 -> 14 -> 7.
        self.stage1 = ConvBNPoolLIF(
            in_channels, dims[0], pool=False, backend=backend
        )
        self.stage2 = ConvBNPoolLIF(
            dims[0], dims[1], pool=False, backend=backend
        )
...
class ALSMFashionMNIST(nn.Module):
    def __init__(
        self,
        *,
        img_size: int = 28,
        ...
        self.patch_embed = SpikingPatchStemFashionMNIST(
            img_size=img_size,
...
def alsm_fashion_mnist(**kwargs) -> ALSMFashionMNIST:
    return ALSMFashionMNIST(**kwargs)


def spikformerLSM(**kwargs) -> ALSMFashionMNIST:
    """Backward-compatible factory for the Fashion-MNIST training API."""
    kwargs = dict(kwargs)
    img_h = kwargs.pop("img_size_h", kwargs.pop("img_size", 28))
    img_w = kwargs.pop("img_size_w", img_h)
    if img_h != img_w:
        raise ValueError("The improved Fashion-MNIST model expects square inputs.")

    kwargs.pop("patch_size", None)
    if "embed_dims" in kwargs:
        kwargs["embed_dim"] = kwargs.pop("embed_dims")
    if "depths" in kwargs:
        kwargs["depth"] = kwargs.pop("depths")
    if "mlp_ratios" in kwargs:
        kwargs["lsm_ratio"] = kwargs.pop("mlp_ratios")

    return ALSMFashionMNIST(img_size=img_h, **kwargs)


__all__ = ["ALSMFashionMNIST", "alsm_fashion_mnist", "spikformerLSM"]
